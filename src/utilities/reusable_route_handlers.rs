...
#[derive(Debug, Serialize, Deserialize)]
pub struct ListAppAuthorizationsResponseBody {
  pub app_authorizations: Vec<AppAuthorization>,
  pub total_count: i64
}

#[derive(Debug, Deserialize)]
pub struct AppAuthorizationListQueryParameters {
  pub query: Option<String>
}
...
pub async fn list_app_authorizations(
  ...
  let http_transaction = http_transaction.clone();
  let list_app_authorizations_action = get_action_from_name("slashstep.appAuthorizations.list", &http_transaction, &state.database_pool).await?;
  let authenticated_principal = get_authenticated_principal(&authenticated_user, &authenticated_app)?;
  verify_principal_permissions(&authenticated_principal, &list_app_authorizations_action, &resource_hierarchy, &http_transaction, &AccessPolicyPermissionLevel::User, &state.database_pool).await?;
  let individual_principal = match &authenticated_principal {
    AuthenticatedPrincipal::User(user) => IndividualPrincipal::User(user.id),
    AuthenticatedPrincipal::App(app) => IndividualPrincipal::App(app.id)
  };
  let query = query_parameters.query.unwrap_or("".to_string());
  let app_authorizations = match AppAuthorization::list(&query, &state.database_pool, Some(&individual_principal)).await {
    ...
    Err(error) => {
      ...
      let http_error = match error {
        ...
        ResourceError::SlashstepQLError(error) => match_slashstepql_error(&error, &DEFAULT_MAXIMUM_ACTION_LIST_LIMIT, "app authorizations"),

        ResourceError::PostgresError(error) => match_db_error(&error, "app authorization"),
  ...
  let app_authorization_count = match AppAuthorization::count(&query, &state.database_pool, Some(&individual_principal)).await {

    Ok(app_authorization_count) => app_authorization_count,
  ...
  let app_authorization_list_length = app_authorizations.len();
  ServerLogEntry::success(&format!("Successfully returned {} {}.", app_authorization_list_length, if app_authorization_list_length == 1 { "app authorization" } else { "app authorizations" }), Some(&http_transaction.id), &state.database_pool).await.ok();
  let response_body = ListAppAuthorizationsResponseBody {
    app_authorizations,
    total_count: app_authorization_count
  };
