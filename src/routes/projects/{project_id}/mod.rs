...
use crate::{
  ...
  utilities::{reusable_route_handlers::delete_resource, route_handler_utilities::{AuthenticatedPrincipal, get_action_by_name, get_app_by_id, get_authenticated_principal, get_project_by_id, get_resource_hierarchy, get_uuid_from_string, verify_delegate_permissions, verify_principal_permissions}}
};
...
async fn handle_get_project_request(
  ...
  let get_projects_action = get_action_by_name("slashstep.projects.get", &http_transaction, &state.database_pool).await?;
