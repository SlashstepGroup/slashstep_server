...
use crate::{
  ...
  resources::{
    access_policy::{AccessPolicyResourceType, ActionPermissionLevel}, action_log_entry::{ActionLogEntry, ActionLogEntryActorType, ActionLogEntryTargetResourceType, InitialActionLogEntryProperties}, app::{App, EditableAppProperties}, app_authorization::AppAuthorization, http_transaction::HTTPTransaction, delegation_policy::DelegationPolicy, server_log_entry::ServerLogEntry, user::User
  }, 
  utilities::{reusable_route_handlers::delete_resource, route_handler_utilities::{AuthenticatedPrincipal, get_action_by_name, get_action_log_entry_expiration_timestamp, get_authenticated_principal, get_delegation_policy_by_id, get_resource_hierarchy, get_uuid_from_string, verify_delegate_permissions, verify_principal_permissions}}
};
...
//   let delegation_policy_id = get_uuid_from_string(&delegation_policy_id, "app", &http_transaction, &state.database_pool).await?;
...
//     |delegation_policy_id, database_pool| Box::new(App::get_by_id(delegation_policy_id, database_pool))
//   ).await;
...
//   let original_target_field = get_app_by_id(&delegation_policy_id, &http_transaction, &state.database_pool).await?;
...
//     actor_type: if let AuthenticatedPrincipal::User(_) = &authenticated_principal { ActionLogEntryActorType::User } else { ActionLogEntryActorType::App },
//     actor_user_id: if let AuthenticatedPrincipal::User(authenticated_user) = &authenticated_principal { Some(authenticated_user.id.clone()) } else { None },
//     actor_delegation_policy_id: if let AuthenticatedPrincipal::App(authenticated_app) = &authenticated_principal { Some(authenticated_app.id.clone()) } else { None },
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    ...
    // .route("/delegation-policies/{delegation_policy_id}", axum::routing::delete(handle_delete_app_request))
    // .route("/delegation-policies/{delegation_policy_id}", axum::routing::patch(handle_patch_app_request))
