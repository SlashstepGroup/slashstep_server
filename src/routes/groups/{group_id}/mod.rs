...
use crate::{
  ...
  utilities::route_handler_utilities::{AuthenticatedPrincipal, get_action_by_name, get_authenticated_principal, get_group_by_id, get_resource_hierarchy, get_uuid_from_string, verify_delegate_permissions, verify_principal_permissions}
};
...
async fn handle_get_group_request(
  ...
  let get_groups_action = get_action_by_name("slashstep.groups.get", &http_transaction, &state.database_pool).await?;
