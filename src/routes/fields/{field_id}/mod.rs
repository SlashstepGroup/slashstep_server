...
use crate::{
  ...
  utilities::{reusable_route_handlers::delete_resource, route_handler_utilities::{AuthenticatedPrincipal, get_action_by_name, get_app_by_id, get_authenticated_principal, get_field_by_id, get_resource_by_id, get_resource_hierarchy, get_uuid_from_string, verify_delegate_permissions, verify_principal_permissions}}
};
...
//   let field_id = get_uuid_from_string(&field_id, "app", &http_transaction, &state.database_pool).await?;
...
//     |field_id, database_pool| Box::new(App::get_by_id(field_id, database_pool))
//   ).await;
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    ...
    // .route("/fields/{field_id}", axum::routing::delete(handle_delete_app_request))
