...
use crate::{
  ...
  utilities::route_handler_utilities::{AuthenticatedPrincipal, get_action_by_name, get_authenticated_principal, get_resource_hierarchy, get_user_by_id, get_uuid_from_string, verify_delegate_permissions, verify_principal_permissions}
};
...
async fn handle_get_user_request(
  ...
  let get_user_action = get_action_by_name("slashstep.users.get", &http_transaction, &state.database_pool).await?;
