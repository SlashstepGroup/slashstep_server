...
async fn verify_list_resources_without_query_and_filter_based_on_requestor_permissions() -> Result<(), TestSlashstepServerError> {
  ...
  // Get the "slashstep.delegation_policies.get" action one time.
  let user = test_environment.create_random_user().await?;
  let get_delegation_policies_action = Action::get_by_name(GET_RESOURCE_ACTION_NAME, &test_environment.database_pool).await?;

  // Grant access to the "slashstep.delegation_policies.get" action to the user for half of the actions.
  ...
  for index in 0..allowed_resource_count {
    ...
    AccessPolicy::create(&InitialAccessPolicyProperties {
      ...
      scoped_resource_type: crate::resources::access_policy::AccessPolicyResourceType::DelegationPolicy,
